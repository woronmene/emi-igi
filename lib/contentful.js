...
const CONTENTFUL_CACHE_SECONDS = 300;
...
function mapEntryToProduct(entry, includes = {}) {
  ...
  // Optional detail fields for sculpture detail page
  const detailIntro = toPlainText(
    fields.detailIntro ??
      fields.detail_intro ??
      fields.longDescription ??
      fields.long_description,
  );
  const delivery = toPlainText(fields.delivery ?? fields.Delivery);
  const authenticity = toPlainText(
    fields.authenticity ?? fields.Authenticity,
  );
  const dimensions = toPlainText(fields.dimensions ?? fields.Dimensions);

  // Optional gallery images (array of assets/links)
  const galleryField =
    fields.galleryImages ??
      ...
      fields.Gallery;
  const gallery =
    Array.isArray(galleryField) && galleryField.length > 0
      ? galleryField
        .map((img) => getImageUrl(img, includes))
        .filter(Boolean)
      : [];

  const category = (
    toPlainText(fields.category ?? fields.Category) || ""
  ).toLowerCase();
...
export async function getProducts() {
  ...
  return unstable_cache(fetchProducts, ["contentful-products"], {
    revalidate: CONTENTFUL_CACHE_SECONDS,
    tags: ["products"],
  })();
}

/**
 * Fetch a single product by slug (or entry ID as fallback) for the detail page.
 * Includes extra fields like delivery/authenticity and gallery images.
 */
async function fetchProductBySlug(slugOrId) {
  ...
  if (!entry) {
    try {
      entry = await client.getEntry(slugOrId, { include: 2 });
    } catch {
      return null;
    }
    assets = [];
  ...
  return mapEntryToProduct(entry, { Asset: assets });
}
...
export async function getProductBySlug(slugOrId) {
  ...
  return unstable_cache(
    ...
    ["contentful-product", slugOrId],
    {
      revalidate: CONTENTFUL_CACHE_SECONDS,
      tags: ["products", `product-${slugOrId}`],
    },
  )();
}
